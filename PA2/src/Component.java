...
public class Component implements Rotatable, Nameable, UpdatingDisplayable, Colorable {
	...
	public void update(final GL2 gl) {
		 ...
		 */
		//translate the component where it will be located on the screen 
		gl.glTranslated(this.position.x(), this.position.y(), this.position.z());
		
		//then want to roate the component around each axis 
		gl.glRotated(this.xAngle, 1, 0, 0);
	    gl.glRotated(this.yAngle, 0, 1, 0);
	    gl.glRotated(this.zAngle, 0, 0, 1);
	    
	    //draw the display which will be represented in the color 
	    if (this.displayable != null) {
	    ...
	    //finally draw the children for the component 
	    for (final Component child : this.children) {
	        child.draw(gl);
	      }
