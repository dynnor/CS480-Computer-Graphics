 ...
 * PA4.java
 * 
 * Author: Dynnor Shebshaievitz (dynnor@bu.edu)
...
if (this.rotate_world) {
	...
	if (magnitude > 0.0001) {
		...
		// normalize to counteract the accumulated round-off error
...
//reminder about Gouraud rendering!! 
/*
 * allow smooth variation with similar flat geometry 
 * normal for each vector we determine the average of the unit normals
 * among all the triangles share vertex or computer normal form surface equations 
 * want to apply illumination eqaution at each vertex to get your colors smoothly interpolate
 * through bilinear interpolation - illuminate discontinuity of common edges in triangles 
 * highlight could be specular 
 */

//reminder about Phong rendering!! 
/*
 * interpolate the nomral vectors (instead of the colors) 
 * application of illumination equation at each pixel 
 */
for (i = 0; i < m - 1; ++i) {
	for (j = 0; j < n - 1; ++j) {
		v0 = mesh.v[i][j];
